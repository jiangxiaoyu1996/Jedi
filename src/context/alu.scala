...
object alu {
  ...
  def equals(args: List[Value]): Value = {
    ...
    if (args2.size == args.size) Boole(args2(0).equals(args2(1)))
       ...
       val args3 = args.map(toReal).filter(_ != None)
       if (args3.size == args.size) Boole(args2(0).equals(args2(1)))
       else {
          val args4 = args.map(toChars).filter(_ != None)
          if (args4.size == args.size) Boole(args2(0).equals(args2(1)))
 ...
 def not(args: List[Value]): Value = {
    if (args(0).isInstanceOf[Integer]) {
      val args2 = args(0).asInstanceOf[Integer]
      -args2
    }
    else if(args(0).isInstanceOf[Real]){
      val args3 = args(0).asInstanceOf[Real]
      -args3
    }
    else if(args(0).isInstanceOf[Boole]){
      val args4 = args(0).asInstanceOf[Boole]
      !args4
    }
    else throw new TypeException("Inputs to unequals must be numbers or bools")
  }
  ...
  // variable ops
   ...
   private def dereference(args: List[Value]) = args(0).asInstanceOf[Variable].getContent
